...
#if defined(ANKI_OSKR)
  const std::string kURL = "anki.bot/oskr";
#elif defined(ANKI_PROD)
  const std::string kURL = "anki.bot/v"
#else
  const std::string kURL = "anki.bot/dev";
#endif
...
void DrawShowPinScreen(AnimationStreamer* animStreamer, const AnimContext* context, const std::string& pin)
{
  ...
  i.DrawTextCenteredHorizontally(pin, CV_FONT_NORMAL, 0.70f, 1, kColor, FACE_DISPLAY_HEIGHT-5, false);
