...
# undef CONSOLE_GROUP
  ...
  static Util::FullEnumToValueArrayChecker::FullEnumToValueArray<ProceduralFace::Parameter, EyeParamInfo,
...
{
  return (FACE_DISPLAY_WIDTH - kProcFace_NominalEyeSpacing)/2;
}
...
void ProceduralFace::SetFacePositionAndKeepCentered(Point<2, Value> center)
{
  ...
  SetFacePosition({CLIP(center.x(), -xmin, FACE_DISPLAY_WIDTH-xmax),
                   CLIP(center.y(), -ymin, FACE_DISPLAY_HEIGHT-ymax)});
}
