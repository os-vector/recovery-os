...
#   undef NUM_FAST_EXP_TERMS
  ...
  {
    DEV_ASSERT_MSG(faceImg.GetNumRows() == FACE_DISPLAY_HEIGHT &&
                   faceImg.GetNumCols() == FACE_DISPLAY_WIDTH,
    ...
    Point2f upperLeft(FACE_DISPLAY_WIDTH, FACE_DISPLAY_HEIGHT);
    ...
    bottomRight.x() = std::min((f32)(FACE_DISPLAY_WIDTH-1), bottomRight.x());
    bottomRight.y() = std::min((f32)(FACE_DISPLAY_HEIGHT-1), bottomRight.y());
                                      ...
                                      Vision::ImageRGB565& output)
  {
    ...
    faceImg.Allocate(FACE_DISPLAY_HEIGHT, FACE_DISPLAY_WIDTH); // Will do nothing if already the right size
    ...
    s32 rowMin = FACE_DISPLAY_HEIGHT-1, rowMax = 0;
    ...
    if(faceData.GetFaceAngle() != 0 || !(faceData.GetFacePosition() == 0) || !(faceData.GetFaceScale() == 1.f)) {
      ...
      SmallMatrix<2, 3, float> W = GetTransformationMatrix(faceData.GetFaceAngle(),
                                                           ...
                                                           static_cast<f32>(FACE_DISPLAY_WIDTH)*0.5f,
                                                           static_cast<f32>(FACE_DISPLAY_HEIGHT)*0.5f);
      
      cv::warpAffine(faceImg.get_CvMat_(), faceImg.get_CvMat_(), W.get_CvMatx_(),
                     cv::Size(FACE_DISPLAY_WIDTH, FACE_DISPLAY_HEIGHT), cv::INTER_NEAREST);
    ...
    // Just to be safe:
    rowMin = Util::Clamp(rowMin, 0, FACE_DISPLAY_HEIGHT-1);
    rowMax = Util::Clamp(rowMax, 0, FACE_DISPLAY_HEIGHT-1);
      ...
      if(nullptr != scanlineDistorter)
      {
        ...
        for(s32 row=rowMin; row < rowMax; ++row)
        {
          ...
          if(shift < 0)
          {
            ...
            memmove(faceImg_row, faceImg_row-shift, FACE_DISPLAY_WIDTH+shift);
            memset(faceImg_row+FACE_DISPLAY_WIDTH+shift, 0, -shift);
          }
          else if(shift > 0)
          {
            Vision::PixelRGB* faceImg_row = faceImg.GetRow(row);
            memmove(faceImg_row+shift, faceImg_row, FACE_DISPLAY_WIDTH-shift);
