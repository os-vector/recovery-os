...
void SoundbankLoader::LoadSoundbankBundleMetadata()
{
  ...
  for ( auto& aBundleInfo : list ) {
    ...
    PRINT_CH_INFO(AudioEngineController::kLogChannelName, "SoundbankLoader.LoadSoundbankBundleMetadata",
                  "Soundbank '%s' %s", aBundleInfo.Path.c_str(),
                  (exists ? "exist" : "do NOT exist"));
    if ( exists ) {
      // Add to map
