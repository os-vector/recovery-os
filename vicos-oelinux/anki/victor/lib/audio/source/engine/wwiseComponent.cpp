...
void WwiseComponent::Terminate()
{
  ...
  AK::IAkStreamMgr* streamManager = AK::IAkStreamMgr::Get();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
static void CallbackFunc( AkCallbackType callbackType, AkCallbackInfo* callbackInfo )
{
...
AudioPlayingId WwiseComponent::PostEvent( AudioEventId eventId, AudioGameObject gameObject, AudioCallbackContext* callbackContext )
{
  ...
  if ( _isInitialized )
  {
    if ( nullptr != callbackContext ) {
      ...
      audioId = AK::SoundEngine::PostEvent( eventId, gameObject, akFlags | AK_EndOfEvent, &CallbackFunc, callbackContext );
    }
    else {
