...
do_compile () {
   ...
   if [[ ${USER_BUILD} == "1" ]]; then
    ...
    if [[ ${OSKR} == 1 ]]; then
      BUILD_FLAVOR_FLAGS="${BUILD_FLAVOR_FLAGS} -DANKI_OSKR=1"
    elif [[ ${PROD} == 1 ]]; then
      BUILD_FLAVOR_FLAGS="${BUILD_FLAVOR_FLAGS} -DANKI_PROD=1"
    fi

   ./project/victor/build-victor.sh -p vicos -c Release -I -v $BUILD_FLAVOR_FLAGS
}
