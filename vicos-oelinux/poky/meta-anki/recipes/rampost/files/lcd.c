...
void lcd_draw_frame2_midas_scale(const uint16_t* frame, size_t size)
{
  static const uint8_t WRITE_RAM = 0x2C;
  lcd_spi_transfer(true, 1, &WRITE_RAM);

  uint16_t scaled_row[LCD_FRAME_WIDTH_MIDAS];

  float scale_x = (float)LCD_FRAME_WIDTH_SANTEK / LCD_FRAME_WIDTH_MIDAS;
  float scale_y = (float)LCD_FRAME_HEIGHT_SANTEK / LCD_FRAME_HEIGHT_MIDAS;

  int i, j;
  for(i = 0; i < LCD_FRAME_HEIGHT_MIDAS; i++) {
    int src_y = (int)(i * scale_y);
    const uint16_t* src_row = frame + (src_y * LCD_FRAME_WIDTH_SANTEK);

    for(j = 0; j < LCD_FRAME_WIDTH_MIDAS; j++) {
      int src_x = (int)(j * scale_x);
      scaled_row[j] = __builtin_bswap16(src_row[src_x]);
    }

    lcd_spi_transfer(false, LCD_FRAME_WIDTH_MIDAS * 2, scaled_row);
  }
}
...
void lcd_draw_frame2(const uint16_t* frame, size_t size)
{
  if(display_version() == MIDAS) {
    lcd_draw_frame2_midas_scale(frame, size);
  } else {
